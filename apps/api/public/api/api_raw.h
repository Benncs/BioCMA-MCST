...
extern "C"
{
  ...
  Param make_params(double biomass_initial_concentration,double final_time,double delta_time,uint64_t number_particle,uint32_t number_exported_result);

  int register_parameters(Handle* handle, Param* params);

  /*Feed*/
  int set_feed_constant(Handle*,double _f,size_t n_species,double* _target,size_t* _species,size_t n_position,size_t * _position, int gas);
