...
// }
template <typename T = void>
requires AutoGenerated::FlagCompileTime::use_cereal_serde void
do_serde(Core::CaseData& case_data)
{
  std::cout << "Serialization..." << std::endl;
  SerDe::save_simulation(case_data);
...
requires AutoGenerated::FlagCompileTime::use_cereal_serde
    ...
    impl_load(std::shared_ptr<IO::Logger>& logger,
              const ExecInfo& exec,
              ...
              std::optional<Simulation::Feed::SimulationFeed> feed)
{
  ...
  try
  {
    ...
    if (logger)
    {
      logger->print("Serde", serde_filename);
    }
...
template <typename T = void>
requires(!AutoGenerated::FlagCompileTime::use_cereal_serde) void do_serde(
    Core::CaseData& /*arg*/)
{
  // NOP
}
...
template <typename T = void>
requires(!AutoGenerated::FlagCompileTime::use_cereal_serde)
    [[maybe_unused]] std::optional<Core::CaseData> impl_load(
