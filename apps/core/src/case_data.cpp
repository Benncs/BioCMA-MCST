...
namespace Core
{
  ...
  {
    // TODO Find orphan profile_section : Kokkos::Profiling::popRegion() called outside  of an actve
    // region. Previous regions:
...
#define RESET "\033[0m"
          ...
          result << GREEN "[Kokkos]: " RESET << line << '\n';
        }
        if constexpr (AutoGenerated::FlagCompileTime::use_mpi)
        {
          result<< GREEN "[Host]: " RESET<<"Number of MPI nodes: " << size << std::endl;
        }

        std::cout << result.str(); // Or use `result` however you need
