...
namespace
{
  ...
  read_options() noexcept
  {
    ...
    auto ceil_power_of_two = [](const std::size_t ppt)
    {
      if (!Common::is_power_of_2(ppt))
      {
        const auto rounded = std::bit_floor(ppt);

        std::printf("[Kernelption] Number of particle needs to be a power of "
                    "two, rounded %ld\r\n",
                    rounded);
    ...
    const auto p_p_t_move
        = Common::read_env_or("BIOMC_PARTICLES_PER_TEAM_MOVE",
                              AutoGenerated::Kernels::particle_per_team_move);

    const auto p_p_t_leave
        = Common::read_env_or("BIOMC_PARTICLES_PER_TEAM_LEAVE",
                              AutoGenerated::Kernels::particle_per_team_leave);

    return { .m_p_p_team_model = ceil_power_of_two(p_p_t_cycle),
             .m_p_p_team_contribs = ceil_power_of_two(p_p_t_contribs),
             .m_p_p_team_move = ceil_power_of_two(p_p_t_move),
             .m_p_p_team_leave = ceil_power_of_two(p_p_t_leave) };
  }
}
...
// FIXME (replace ifdef by forward declaration)
               ...
               std::optional<std::size_t> force_run_id)
  {
    ...
    info.kernel_options = read_options();
