...
namespace Core
{
  GlobalInitialiser::GlobalInitialiser(const ExecInfo& _info, UserControlParameters _user_params)
      : info(_info), user_params(std::move(_user_params)), particle_per_process(0),
  ...
  OptionalPtr<CmaRead::FlowIterator> GlobalInitialiser::init_flow_iterator()
  {
    ...
    try
    {
      ...
      std::cout << IO::AnsiCode::blue<< "[Initializer]: "<< IO::AnsiCode::reset<< "Flowmap loaded: " << flow_handle->size()
                << std::endl;
      ...
      std::cout << IO::AnsiCode::blue<< "[Initializer]: " <<IO::AnsiCode::reset<< "Flowmap loaded with " << state->n_compartments
  ...
  std::optional<Simulation::ScalarInitializer> GlobalInitialiser::init_scalar()
  {
    ...
    {
      std::cerr << IO::AnsiCode::red<< "[Initializer]: " <<IO::AnsiCode::reset<<  "WARNING: using Default Initialiser" << std::endl;
