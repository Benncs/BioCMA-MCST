...
namespace Core
{
  ...
  {
    std::string case_name = cma_case_path + "/cma_case";
  ...
  OptionalPtr<Simulation::SimulationUnit> GlobalInitialiser::init_simulation()
  {
    ...
    {
      return init_simulation(std::move(*mc), *scalar, feed);
    }
    return std::nullopt;
  }
  ...
  GlobalInitialiser::init_transitionner(std::unique_ptr<CmaRead::FlowIterator> &&flow_handle)
  {
    ...
    std::cout << "gas transitionner " << f_init_gas_flow << std::endl;
  ...
  {
    if (!check_steps(InitStep::FlowIterator, InitStep::Transitioner, InitStep::Scalar, InitStep::MC))
    {
      return std::nullopt;
    }
