...
#  define FILL_PAYLOAD                                                                                                 \
    ...
    {                                                                                                                  \
      ...
      mpi_payload.neighbors = current_reactor_state->liquid_flow.getViewNeighors().to_const();                          \
    }
#else
#  define FILL_PAYLOAD
#endif // MO_MPI
...
#  define MPI_DISPATCH_MAIN                                                                                            \
    ...
    {                                                                                                                  \
      ...
      {                                                                                                                \
        WrapMPI::send(WrapMPI::SIGNALS::RUN, __macro_j);                                                                   \
               ...
               Core::PartialExporter &partial_exporter)
{
  ...
  auto gas_volume =
      (!gas_concentration.has_value()) ? std::nullopt : std::make_optional(current_reactor_state->gasVolume);

  main_exporter->update_fields(
      current_time, simulation.getCliqData(), current_reactor_state->liquidVolume, gas_concentration, gas_volume);
  ...
  gas_volume =
      (!simulation.getCgasData().has_value()) ? std::nullopt : std::make_optional(current_reactor_state->gasVolume);
...
// NOLINTEND
{
  ...
  if (++dump_counter == dump_interval)
  {
    SEND_MPI_SIG_DUMP
    auto vg =
        (simulation.getCgasData().has_value()) ? std::make_optional(current_reactor_state->gasVolume) : std::nullopt;

    update_progress_bar(n_iter_simulation, __loop_counter);
    main_exporter->update_fields(
        current_time, simulation.getCliqData(), current_reactor_state->liquidVolume, simulation.getCgasData(), vg);
