...
static void handle_export(size_t &dump_counter,
               ...
               std::unique_ptr<DataExporter> &exporter)
{
  ...
  {
    auto probes = Simulation::Probes(0, dump_interval, {"exit_time"});
    simulation.set_probes(std::move(probes));
  }
  ...
  if constexpr (AutoGenerated::FlagCompileTIme::use_probe)
  {
    ...
    const double *const probe_ptr = simulation.probes.raw_get();
    exporter->append_probes(Simulation::Probes::buffer_size, probe_ptr);

    simulation.probes.clear();
  }

  transitioner.reset();
}

void handle_export(size_t &dump_counter,
