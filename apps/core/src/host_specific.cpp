...
static void main_loop(const Core::SimulationParameters &params,
                      const ExecInfo &exec,
...
inline static void update_progress_bar(size_t total, size_t current_position)
{
  ...
  if constexpr (AutoGenerated::FlagCompileTime::verbose)
  {
    ...
    std::cout << "Progress: [" << progressBar << "] " << std::fixed << std::setprecision(2)
              << (static_cast<float>(current_position) * 100.0 / static_cast<float>(total)) << "%\r" << std::flush
...
#  define FILL_PAYLOAD                                                                                                                               \
    ...
    {                                                                                                                                                \
      mpi_payload.liquid_flows = current_reactor_state->liquid_flow.getViewFlows().data();                                                           \
      mpi_payload.liquid_volumes = current_reactor_state->liquidVolume;                                                                              \
      mpi_payload.gas_volumes = current_reactor_state->gasVolume;                                                                                    \
      mpi_payload.neigbors = current_reactor_state->liquid_flow.getViewNeighors().to_const();                                                        \
    }
#else
#  define FILL_PAYLOAD
#endif // MO_MPI
...
std::unique_ptr<CORE_DE::MainExporter> make_main_exporter(const ExecInfo &exec, const Core::SimulationParameters &params)
{
               ...
               CORE_DE::PartialExporter &partial_exporter)
{
  ...
  const size_t dump_number = std::min(n_iter_simulation, static_cast<size_t>(params.user_params.number_exported_result)) - 1;
  ...
  auto gas_volume = (!gas_concentration.has_value()) ? std::nullopt : std::make_optional(current_reactor_state->gasVolume);

  main_exporter->update_fields(current_time, simulation.getCliqData(), current_reactor_state->liquidVolume, gas_concentration, gas_volume);
  ...
  auto loop_functor = [&](auto &&local_container)
  {
    ...
    for (size_t __loop_counter = 0; __loop_counter < n_iter_simulation; ++__loop_counter)
    {
      ...
      if (SignalHandler::is_usr1_raised())
      {
        ...
        //   local_container.serde(ar);
        // wrap_ser(std::forward<std::remove_reference_t<decltype(local_container)>>(local_container));
      }
    }
  };
  ...
  // SerDe::save_particle(*simulation.mc_unit);


  gas_volume = (!simulation.getCgasData().has_value()) ? std::nullopt : std::make_optional(current_reactor_state->gasVolume);

  main_exporter->update_fields(current_time, simulation.getCliqData(), current_reactor_state->liquidVolume, simulation.getCgasData(), gas_volume);
...
// NOLINTEND
{
  ...
  {
    SEND_MPI_SIG_DUMP
    auto vg = (simulation.getCgasData().has_value()) ? std::make_optional(current_reactor_state->gasVolume) : std::nullopt;

    update_progress_bar(n_iter_simulation, __loop_counter);
    main_exporter->update_fields(current_time, simulation.getCliqData(), current_reactor_state->liquidVolume, simulation.getCgasData(), vg);
