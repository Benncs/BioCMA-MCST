...
// In multirank context, fill the struct that will be broadcast to other
// workers
#  define FILL_PAYLOAD                                                         \
    if constexpr (AutoGenerated::FlagCompileTime::use_mpi)                     \
    {                                                                          \
      mpi_payload.fill(new_current);                                           \
    }
...
#  define MPI_DISPATCH_MAIN                                                    \
    if constexpr (AutoGenerated::FlagCompileTime::use_mpi)                     \
    {                                                                          \
      const auto _ = mpi_payload.sendAll(exec.n_rank);                         \
      (void)_;                                                                 \
    }
...
namespace
{
  ...
  void main_loop(const std::shared_ptr<IO::Logger>& logger,
                 ...
                 CmaUtils::TransitionnerPtrType d_transionner,
...
void
host_process(std::shared_ptr<IO::Logger> logger,
             const ExecInfo& exec,
             Simulation::SimulationUnit& simulation,
             const Core::SimulationParameters& params,
