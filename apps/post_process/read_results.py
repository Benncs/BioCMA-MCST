...
class MainExportResult(RawResults):
    ...
    time: np.ndarray
    concentrations_liq: np.ndarray
    volumes_liq: np.ndarray
    ...
    def _read(self, file):
        ...
        self.concentrations_liq = np.array(file.get("/records/concentration_liquid"))
        self.volumes_liq = np.array(file.get("/records/gas_volume"))

        opt = file.get("/records/concentration_gas")
        if opt is not None:
            self.concentrations_gas = opt

        opt = file.get("/records/gas_volume")
        if opt is not None:
            self.volumes_gas = opt
        ...
        self.n_export = self.concentrations_liq.shape[0]
        self.n_compartment = self.concentrations_liq.shape[1]
        self.n_species = self.concentrations_liq.shape[2]
...
class PartialResult(RawResults):
    ...
    def _read_bioparam(self, file):
        ...
        for ds_name in file.get("biological_model"):
            node = file.get("biological_model")[ds_name]
            d = {}
            for key in node:
...
def read_file(file_name: str):
    ...
    repartition = np.zeros((main.n_export, main.n_compartment))
    ...
    return Results(main, partials, repartition)
