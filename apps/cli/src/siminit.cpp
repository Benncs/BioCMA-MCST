...
static const ScalarFactory::ScalarVariant DefaultIntialiser = ScalarFactory::Uniform({1.});

static const ScalarFactory::ScalarVariant DefaultIntialiserTPF =
    ScalarFactory::Local({5., 0.}, {0}, std::vector<double>({0, 0.}), std::vector<size_t>({0}));
...
init_state(Core::SimulationParameters &params, std::unique_ptr<CmaRead::FlowIterator> &flow_handle, const CmtCommons::cma_exported_paths_t &paths);
...
init_simulation(const ExecInfo &info, Core::SimulationParameters &params, std::unique_ptr<Simulation::FlowMapTransitioner> &transitioner)
{
  ...
  init_host_only(info, params, _flow_handle, liquid_neighbors, liquid_volume, gas_volume, worker_neighbor_data);
  ...
  if (info.current_rank != 0)
  {
    ...
    liquid_neighbors = CmaRead::Neighbors::Neighbors_const_view_t(worker_neighbor_data, params.n_compartments, n_col);
  }
  ...
  auto mc_unit = AutoGenerated::wrap_init_model_selector(
      i_model, info, user_params.number_particle, liquid_volume, liquid_neighbors, user_params.biomass_initial_concentration);

  // We init gas flow specific object only on the host and if user defined two
  // phase flow simulation
  const bool f_init_gas_flow = info.current_rank == 0 && params.is_two_phase_flow;
  ...
  {
    arg = ScalarFactory::File(params.n_compartments, params.user_params.initialiser_path);
  }

  Simulation::ScalarInitializer scalar_init = ScalarFactory::scalar_factory(f_init_gas_flow, gas_volume, liquid_volume, arg);
  ...
  // else
  // {
  //    scalar_init = ScalarFactory::scalar_factory(
  //     false, gas_volume, liquid_volume, arg);
  // }

  Simulation::Feed::SimulationFeed feed = init_feed();

  // Construct the main simulation object (one per rank)
  auto simulation = std::make_unique<Simulation::SimulationUnit>(std::move(mc_unit), scalar_init, feed);

  // FIXME
  auto dim = simulation->getDimensions();
  if (i_model == 4 && dim.n_species < 4)
  {
  ...
  transitioner = std::make_unique<Simulation::FlowMapTransitioner>(
      params.n_different_maps, params.n_per_flowmap, Simulation::FlowMapTransitioner::Discontinuous, n_t, std::move(_flow_handle), f_init_gas_flow);

  std::cout << info.current_rank << ": INIT OK" << std::endl;
                           ...
                           std::vector<size_t> &worker_neighbor_data)
{
  ...
  if (liq_volume.size() > 1)
  {
    ...
    params.d_t = (params.user_params.delta_time == 0.) ? _flow_handle->MinLiquidResidenceTime() / 100. : params.user_params.delta_time;
  }
  else
  {
  ...
  const auto n_per_flowmap =
      (t_per_flowmap == 0 || params.n_different_maps == 1) ? 1 : static_cast<size_t>(t_per_flowmap / static_cast<double>(params.d_t)) + 1;
...
static Simulation::Feed::SimulationFeed init_feed()
{
  ...
  constexpr double flow = 0.15 / 3600 * 20e-3; // 0.77/3600*20e-3;//3.5e-6;
  ...
  Simulation::Feed::FeedDescritor fg(0.1 / 3600 * 20e-3, {300e-3}, {0}, {1}, Simulation::Feed::Constant{});
