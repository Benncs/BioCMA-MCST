 ...
 static void write_attributes(HighFive::File &file)
  {
    ...
    file.createAttribute<std::string>("description", std::string("Interesting results"));
  }

  static void write_initial(HighFive::File &file,ExecInfo& info,SimulationParameters& params)
  { 
    size_t n_0_particle = info.n_rank*params.n_particles;

    file.createDataSet("initial_parameters/number_particles_0", n_0_particle);
    file.createDataSet("initial_parameters/number_compartment", params.n_compartments);
...
DataExporter::DataExporter(ExecInfo& info ,SimulationParameters& params,std::string_view _filename) : filename(_filename.data())
