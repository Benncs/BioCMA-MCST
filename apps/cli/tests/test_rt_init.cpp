...
static ExecInfo c_test_set_openmp_threads(int rank,int size,int n_threads)
{
    ...
    set_openmp_threads(rank, size, info, params);
     std::cerr<<"Number of threads: "<<info.thread_per_process<<std::endl;
    return info;
}

// Test case 1: We have 4 nodes with 8 threads, so each node should have 2 threads
...
// Test case 2: One node with 16 threads, should have number of threads equal to the number of CPU cores
...
// Test case 3: 5 nodes and we are in the last (4), as 16%5 = 1, the last should have int(16/5 + 1) = 4 threads
...
// Test case 4: Same situation as test 3 but we are not on the last node, so we have int(16/5) = 3 threads
...
// Test case 6: 2 nodes and we are in the first (0), so it should have int(8/2) = 4 threads
...
// Test case 7: 3 nodes and we are in the second (1), so it should have int(12/3) = 4 threads
