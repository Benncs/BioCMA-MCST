...
namespace
{
          ...
          && AutoGenerated::FlagCompileTime::use_cereal_serde>::type>
  {
    ...
    static void
    serialize(Archive& ar, const ViewType& view, std::size_t n_used_elements)
    {
      using T = typename ViewType::value_type;
      auto temp_host = Kokkos::create_mirror_view_and_copy(
          Kokkos::DefaultHostExecutionSpace(), view);
      std::vector<T> data_vector(temp_host.data(),
                                 temp_host.data() + n_used_elements);
      ar(data_vector);
    }

    template <class Archive>
    static void
    deserialize(Archive& ar, ViewType& view)
    {
      using T = typename ViewType::value_type;
...
// Helper to check if type can be serialized
...
// Helper to serialize Kokkos View in any memory space
template <class Archive, class ViewType>
void
serialize_view(Archive& archive, const ViewType& view)
{
  // using T = typename ViewType::value_type;
  ViewSerialization<ViewType>::serialize(archive, view, view.extent(0));
}

// Helper to deserialize Kokkos View in any memory space
template <class Archive, class ViewType>
void
deserialize_view(Archive& archive, ViewType& view)
{
