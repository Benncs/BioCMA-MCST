...
namespace CommonMaths
{
  ...
  {
    // NOLINTBEGIN
    // std::bit_cast keeps the same exact bit in the target type
    // better than unsafe C const
    unsigned int bx = Kokkos::bit_cast<unsigned int>(x);
    const unsigned int ex = bx >> 23;
    const signed int t
        = static_cast<signed int>(ex) - static_cast<signed int>(127);
    // unsigned int s = (t < 0) ? (-t) : t;
    bx = 1065353216 | (bx & 8388607);
    x = Kokkos::bit_cast<float>(bx);
