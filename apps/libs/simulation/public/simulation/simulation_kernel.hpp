...
namespace Simulation::KernelInline
{
  ...
  public:
    ...
    Kernel(double _d_t,
        ...
        : d_t(_d_t), list(_list), extra(std::move(_extra)),
          local_compartments(std::move(_local_compartments)), move_info(std::move(_move_info)), rng_pool(pool),
  ...
  private:
    ...
    KOKKOS_FUNCTION void handle_division(auto& particle) const
    {
  ...
  *****************/
  ...
  KOKKOS_FUNCTION void Kernel<ListType>::operator()(const Kokkos::TeamPolicy<ComputeSpace>::member_type& team_handle,int& dead_count) const
  {
    ...
    particle.update(d_t, local_compartments(properties.current_container).concentrations, list.rng_instance);
  ...
  // template <typename ListType>
  // KOKKOS_FUNCTION void Kernel<ListType>::operator()(const std::size_t i_particle,int& dead_count) const
  // {
