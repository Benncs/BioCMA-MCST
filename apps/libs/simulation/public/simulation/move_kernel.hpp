...
namespace Simulation::KernelInline
{
  ...
  template <typename ExecSpace> struct MoveInfo
  {
    ...
    MoveInfo()
        : diag_transition("diag_transition", 0), leaving_flow("leaving_flow", 0),
                          ...
                          const double random_number)
  {
    ...
    std::size_t next = neighbors(i_compartment, 0); // Default to the first neighbor
  ...
  struct MoveFunctor
  {
    MoveFunctor(double _d_t,
                ...
                MC::ParticleAges _ages,bool b_move,bool b_leave)
        : d_t(_d_t), positions(std::move(p)), n_particles(n_p), move(std::move(m)),
          ...
          probes(std::move(_probes)), ages(std::move(_ages)),enable_move(b_move),enable_leave(b_leave) {};
