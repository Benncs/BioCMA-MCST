...
namespace Simulation::KernelInline
{
  ...
  template <typename Space, ModelType Model> struct CycleFunctors
  {
           ...
           MC::DomainState<ComputeSpace>&& new_move)
    {
      ...
      // launch_move is skipped entirely when need_launch() is false, and
      // post_cycle tallies the reducer either way: clear it here so a step that
      // launches nothing cannot re-count the previous step's events.
      Kokkos::deep_copy(move_reducer, MoveLeaveTally{ 0, 0 });
    ...
    get_host_reduction()
    {
      ...
      const auto move_tally
          = Kokkos::create_mirror_view_and_copy(HostSpace(), move_reducer)();

      if constexpr (AutoGenerated::FlagCompileTime::enable_event_counter)
      {
        move_kernel.events.add<MC::EventType::Move>(move_tally.moved);
        move_kernel.events.add<MC::EventType::Exit>(move_tally.dead);
      }

      return std::tuple(host_red, move_tally.dead);
    }
