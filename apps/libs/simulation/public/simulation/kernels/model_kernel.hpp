...
namespace Simulation::KernelInline
{
  ...
  template <ModelType M> struct CycleFunctor
  {
               ...
               value_type& reduce_val) const
    {
      ...
      const auto& model = particles.model;
      const auto& contribs = particles.contribs;
      ...
      Kokkos::parallel_reduce(
          ...
          [&](std::size_t relative_index, value_type& lv)
          {
            ...
            if (active)
            {
              const auto pos = particles.position(flatten_index);
              ...
              {
                division(flatten_index, lv);
              }
              else
              {
                particles.ages(flatten_index, 1) += d_t;
              }
            }
          },
          local);
    ...
    KOKKOS_INLINE_FUNCTION void
    division(const std::size_t idx, value_type& reduce_val) const
    {
