...
namespace Simulation
{
  ...
  public:
    ...
    void set_mtr_model(MassTransfer::Type::MtrTypeVariant&& variant);
    ...
    void post_init_compartments();
      ...
      void set_logger(std::shared_ptr<IO::Logger>);
  ...
  void SimulationUnit::cycleProcess(auto&& container, double d_t)
  {
    ...
    if (_waiting_allocation_particle != 0)
    {
      if(logger)
      {
        logger->alert("Simulation", "Overflow of particle not implemented yet (ignore _waiting_allocation_particle)");
      }
