...
namespace MC
{
                   ...
                   double& total_mass)
    {
      ...
      Kokkos::parallel_reduce(
          ...
          KOKKOS_LAMBDA(const int i, double& local_mass) {
            container.position(i) = rng.uniform_u(min_c, max_c);
            Model::init(i, container.model);
  ...
  inline void post_init_weight(std::unique_ptr<MonteCarloUnit>& unit, double x0, double total_mass)
  {
    ...
    //   / (total_mass); const double new_weight = (x0 * unit->domain.getTotalVolume()) /
    //   (total_mass); KOKKOS_ASSERT(new_weight > 0);
