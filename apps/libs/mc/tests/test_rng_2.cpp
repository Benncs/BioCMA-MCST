...
void
check_moment(double empirical, double theoretical, double tol, const char* name)
{
...
template <int N,
          typename T,
          typename Device,
          MC::Distributions::ProbabilityLaw<T, Device> F>
void
moment_test(F dist, std::string name, T tol = tolerance, int seed = test_seed)
{
  ...
  T skew = (sum_skew / N - 3 * mean * var - mean * mean * mean)
           / std::pow(var, 1.5);
...
main()
{
  ...
  moment_test<N, double, Device>(MC::Distributions::Normal<double>{ 0, 1 },
                                 "Normal");

  std::cerr << "Normal2" << std::endl;

  moment_test<N, double, Device>(MC::Distributions::Normal<double>{ mu, sigma },
                                 "Normal2");
  ...
  moment_test<N, double, Device>(
      MC::Distributions::SkewNormal<double>{ xi, omega, alpha }, "Skew-Normal");

  std::cerr << "Truncated-Normal" << std::endl;

  moment_test<N, double, Device>(
      MC::Distributions::TruncatedNormal<double>{ 1., 0.33, 0., 5. },
      "Truncated-Normal");

  std::cerr << "Truncated-Normal2" << std::endl;
  moment_test<N, double, Device>(
      MC::Distributions::TruncatedNormal<double>(-5., 1.4, -10., 1.),
      "Truncated-Normal2");
  ...
  constexpr auto length_c_dist = MC::Distributions::TruncatedNormal<double>(
      l_min_um, l_min_um / 5., 0.5 * l_min_um, l_max_um);

  for (int i = 0; i < 5; ++i)
  {
    moment_test<N, double, Device>(
        length_c_dist,
        "double Truncated-Normal3",
        0.2,
        AutoGenerated::MC::debug_MC_RAND_DEFAULT_SEED);
  }
  ...
  //  omega = 3./7.;
  //  alpha = 4;
  //  moment_test<N,Device>(MC::SkewNormal<double>{xi, omega,
  //  alpha},"Skew-Normal2"); std::cerr << "Skew-Normal3" << std::endl; xi =
  //  -1.; omega = 1.; alpha = -7;
  //  moment_test<N,Device>(MC::SkewNormal<double>{xi, omega,
  //  alpha},"Skew-Normal3");
  ...
  constexpr auto dexp = MC::Distributions::Exponential<float>(lambda);

  for (int i = 0; i < 5; ++i)
  {
    moment_test<N, float, Device>(
        dexp,
        "float Exponential",
        0.2,
        AutoGenerated::MC::debug_MC_RAND_DEFAULT_SEED);
