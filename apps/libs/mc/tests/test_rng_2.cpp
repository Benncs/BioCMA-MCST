...
void check_moment(double empirical,
                  double theoretical,
                  double tol,
                  const char* name)
{
                 ...
                 int seed = test_seed)
{
  ...
  T skew =
      (sum_skew / N - 3 * mean * var - mean * mean * mean) / std::pow(var, 1.5);
...
int main()
{
  ...
  for (int i = 0; i < 5; ++i)
  {
    moment_test<N, double, Device>(length_c_dist,
                                   "double Truncated-Normal3",
                                   0.2,
                                   AutoGenerated::debug_MC_RAND_DEFAULT_SEED);
  }
  ...
  for (int i = 0; i < 5; ++i)
  {
    moment_test<N, float, Device>(dexp,
                                  "float Exponential",
                                  0.2,
                                  AutoGenerated::debug_MC_RAND_DEFAULT_SEED);
  }
