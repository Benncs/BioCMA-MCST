...
//   const std::size_t size = 1000;
//   const std::size_t ndiv = 10;
//   MC::ParticlesContainer<M> container(MC::load_tuning_constant(), size, 4);
//   MC::pool_type rng;
//   Kokkos::parallel_for(
//       "spawn", ndiv, KOKKOS_LAMBDA(const int i) {
//         KOKKOS_ASSERT(container.handle_division(rng, i));
...
//                 && container.position.extent(0) ==
//                 container.status.extent(0));
...
//   MC::ParticlesContainer<M> container(MC::load_tuning_constant(), size, 4);
//   MC::pool_type rng(AutoGenerated::MC::debug_MC_RAND_DEFAULT_SEED);
//   Kokkos::parallel_for(
...
//   Kokkos::fence();
//   KOKKOS_ASSERT(container.n_particles() == size);
//   container.remove_inactive_particles(to_remove);
