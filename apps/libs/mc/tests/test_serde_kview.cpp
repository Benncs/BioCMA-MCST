...
#include <iostream>
template <typename
          = std::enable_if_t<AutoGenerated::FlagCompileTime::use_cereal_serde>>
void
test_1d()
{
  ...
  Kokkos::parallel_for(
      "initialize", n, KOKKOS_LAMBDA(const int i) { view(i) = i; });
  Kokkos::fence();
...
test_2d()
{
  ...
  Kokkos::parallel_for(
      "verify", n, KOKKOS_LAMBDA(const int i) {
        for (std::size_t j = 0; j < n_p; ++j)
        {
          KOKKOS_ASSERT(Kokkos::abs(new_view(i, j) - float(i) * float(j))
                        < 1e-6);
        }
      });
  Kokkos::fence();
