...
namespace
{
   ...
   *
   * This functor increments the count of particles in their respective
   * containers.
   *
  ...
  template <typename ScatterType> struct NcellFunctor
  {
     ...
     */
    KOKKOS_FUNCTION
    NcellFunctor(MC::ParticlePositions _list,
                 MC::ParticleStatus _status,
                 ScatterType _n_cells)
        : n_cells(std::move(_n_cells)), positions(std::move(_list)),
  ...
  template <ModelType Model> struct InitFunctor
  {
    explicit InitFunctor(MC::ParticlesContainer<Model> _list,
                         ...
                         MC::KPRNG kprng,
                         Model::Config config)
        : particles(std::move(_list)), rng(std::move(kprng)), min_c(min),
          max_c(max), config(std::move(config))
    {
    }
  ...
  void
  initialize_model(const ExecInfo& info,
                   ...
                   double& total_mass)
  {
    ...
    typename Model::Config config
        = Models::get_model_configuration<Model>(info, n_particles);
...
namespace MC
{
            ...
            bool uniform_init)
  {
    ...
    auto visitor = [&, _min_c = min_c, _max_c = max_c](auto&& container)
    {
      ...
      initialize_model(
          info, n_particles, container, _min_c, _max_c, rng, total_mass);
      Kokkos::fence();
    };
