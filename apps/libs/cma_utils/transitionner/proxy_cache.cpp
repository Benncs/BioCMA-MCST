...
{
  void
  ProxyPreCalculatedHydroState::set_diag_transition(std::vector<double>&& diag)
  {
  ...
  void
  ProxyPreCalculatedHydroState::set_transition_matrix(FlowMatrixType&& matrix)
  {
  ...
  {
    state.cumulative_probability =
        get_cumulative_probabilities(neighbors, state.transition_matrix);
  }
