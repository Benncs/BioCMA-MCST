...
namespace CmaUtils
{
  ...
  {
    return NeighborsView<HostSpace>(
        const_cast<size_t*>(liquid_neighbors.data().data()),
        liquid_neighbors.getNRow(),
        liquid_neighbors.getNCol());
  }
  FlowMapTransitionner::FlowMapTransitionner(
      size_t _n_flowmap,
      size_t _n_per_flowmap,
      size_t number_time_step,
      std::unique_ptr<CmaRead::FlowIterator>&& _iterator,
      bool is_two_phase_flow)
      : two_phase_flow((is_two_phase_flow)), n_per_flowmap(_n_per_flowmap),
        n_flowmap(_n_flowmap), n_timestep(number_time_step),
      ...
      CmaUtils::ProxyPreCalculatedHydroState& gas_hydro_state) const
  {
    ...
    liq_hydro_state.state.inverse_volume =
    ...
    if (is_two_phase_flow())
    {
      ...
      gas_hydro_state.state.inverse_volume =
    ...
    //  CmtCommons::CMAExportType::GasFlow); std::cout<<"test
    //  "<<test.has_value()<<std::endl;
  }

  IterationState FlowMapTransitionner::advance()
  {
    update_flow();
    auto liquid_neighbors =
        get_current_reactor_state().liquid_flow.getViewNeighors();
    ...
    return common_advance(host_view,
                          {{"energy_dissipation",
                            get_current_reactor_state().energy_dissipation}});
  }

  IterationState FlowMapTransitionner::advance_worker(
      std::span<double> flows,
      std::span<double> volumeLiq,
      std::span<double> volumeGas,
      const CmaRead::Neighbors::Neighbors_const_view_t& neighbors)
  {
    auto host_view =
        NeighborsView<HostSpace>(const_cast<size_t*>(neighbors.data().data()),
                                 neighbors.getNRow(),
                                 neighbors.getNCol());
  ...
  void FlowMapTransitionner::update_counters()
  {
    ...
    if (++this->current_flowmap_count == this->n_per_flowmap)
    {
      ...
      // We reset it to 0 and increment repetition_count, this allow to switch
      // to the next element in buffer During the first loop we have
      // repetition_count<n_flowmap but after looping we have
      // repetition_count>n_flowmap
  ...
  FlowMapTransitionner::get_n_timestep() const noexcept
  {
    return n_timestep;
  }
} // namespace CmaUtils
