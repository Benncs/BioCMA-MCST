...
{
  /**
   * @brief Proxy class for PreCalculatedHydroState  used to fill this struct
  ...
  public:
    ...
    /**
     * @brief Set the transition matrix using an rvalue reference to
     * FlowMatrixType.
     ...
     */
    void set_cumulative_probability(
        const CmaRead::Neighbors::Neighbors_const_view_t& neighbors);

    /**
     * @brief Set the diagonal elements of the transition matrix.
     * @param diag Rvalue reference to a vector of double containing diagonal
     * elements.
     */
    void set_diag_transition(std::vector<double>&& diag);

    PreCalculatedHydroState state; ///< Instance of PreCalculatedHydroState
                                   ///< containing the filled data.
  };
} // namespace CmaUtils
