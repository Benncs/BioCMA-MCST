...
#define _INDICES(__name__) static_cast<std::size_t>(Indices::__name__)
  ...
  struct Yields
  {
    ...
    static constexpr double nu_s_x = MolarMass::glucose / (MolarMass::X) * (Yields::Y_EG + Yields::Yo_EG) / (Yields::Y_XG * Yields::Yo_EG);
  };
...
namespace Models
{
  ...
  KOKKOS_FUNCTION void UptakeAcetate::metabolism()
  {
    ...
    static constexpr double y2_b = (Yields::Y_EG + Yields::Yo_EG) / (Yields::Y_EG * Yields::Y_OG);  // G/O
    ...
    static constexpr double y2_e = (Yields::Y_EA + Yields::Yo_EA) / (Yields::Yo_EA * Yields::Y_OA); // A/O
    static constexpr double yield_mol_S_O = 1.; // Number of mol of S from mole of O for mode D conversion

    const auto [phi_s_in, phi_a_in, phi_o2_in] = phi_uptakes; // kg/s
    const double current_mass = mass();

    const double phi_s_in_mol = phi_s_in / (MolarMass::glucose); // mol/s
    const double phi_a_in_mol = phi_a_in / (MolarMass::acetate);
    const double phi_o2_in_mol = phi_o2_in / (MolarMass::dioxygen);

    const double r1_max = nu * factor * Yields::x_s / MolarMass::glucose; // molS/s
    ...
    const double phi_o2_residual = phi_o2_in_mol - (Yields::Y_EG * Yields::Y_OG) / (Yields::Y_EG + Yields::Yo_EG) * phi_mode_b; // molO2/s

    const double nu_residual = (Yields::Y_EG + Yields::Yo_EG) / Yields::Yo_EG * r1_max - phi_mode_b; // molS/s
    ...
    rates.nu = (Yields::Y_XG * Yields::Yo_EG / (Yields::Y_EG + Yields::Yo_EG) * phi_mode_b +
  ...
  KOKKOS_FUNCTION double UptakeAcetate::uptake(const LocalConcentrationView &concentration)
  {
    ...
    phi_uptakes.acetate = 1 / micro_mixing * concentration(_INDICES(Ac)) / 1000.;
    phi_uptakes.oxygen = 1 / micro_mixing * concentration(_INDICES(O2)) / 1000.;
    return gamma_PTS_S;
  }

  KOKKOS_FUNCTION void UptakeAcetate::update(double d_t, MC::ParticleDataHolder &p, const LocalConcentrationView &concentration, MC::KPRNG _rng)
  {
    const double s = Kokkos::max(0., concentration(0));
    const double gamma_PTS_S = uptake(concentration);

    const double nu_p = phi_uptakes.glucose / factor / (Yields::nu_s_x);

    metabolism();
    this->lenght += d_t * rates.nu;
