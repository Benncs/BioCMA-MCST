...
namespace WrapMPI
{
   ...
   *
   * This function sends a single object of type `DataType` to the specified
   * destination.
   *
   * @tparam DataType A type satisfying the `POD` concept.
   ...
   *
   * This function sends a collection of objects of type `DataType` to the
   * specified destination. Optionally, the size of the collection can also be
   * transmitted.
   *
   * @tparam DataType A type satisfying the `POD` concept.
   ...
   *
   * This function attempts to receive a single object of type `DataType` from
   * the specified source. If the receive operation fails, it returns an empty
   * `std::optional`.
   *
   * @tparam DataType A type satisfying the `POD` concept.
   * @param src The source identifier from which to receive data.
   * @param status Optional pointer to an `MPI_Status` structure to retrieve
   * message details (default is `nullptr`).
   * @param tag Optional tag to filter messages (default is 0).
   * @return A `std::optional` containing the received data if successful, or
   ...
   *
   * This function attempts to receive data into the provided span buffer. The
   * buffer must be pre-allocated and of sufficient size to hold the incoming
   * data.
   *
   * @tparam DataType A type satisfying the `POD` concept.
   ...
   *
   * This function attempts to receive a single object of type `DataType` from
   * the specified source. If the operation fails, it throws an exception.
   *
   * @tparam DataType A type satisfying the `POD` concept.
   * @param src The source identifier from which to receive data.
   * @param status Optional pointer to an `MPI_Status` structure to retrieve
   ...
   *
   * This function attempts to receive a vector of objects of type `T` from the
   * specified source. If the receive operation fails, it returns an empty
   * `std::optional`.
   *
   ...
   * message details (default is `nullptr`).
   * @param tag Optional tag to filter messages (default is 0).
   * @return A `std::optional` containing the received vector if successful, or
   * empty if the operation fails.
   */
  template <POD_t T>
   ...
   *
   * This function attempts to receive a vector of objects of type `T` from the
   * specified source. If the operation fails, it throws an exception.
   *
   * @tparam T The type of data to receive.
   * @param src The source identifier from which to receive data.
   * @param status Optional pointer to an `MPI_Status` structure to retrieve
   ...
   *
   * This function sends raw data of type `T` from the root process to all other
   * processes. It performs only basic safety checks
   *
   ...
   * @param root The identifier of the root process that initiates the
   * broadcast.
   * @return An integer indicating the success or failure of the operation.
   *
   ...
   *
   * This function sends a single object of type `DataType` from the root
   ...
   *
   * This function sends data stored in a `std::span` from the root process to
   * all other processes.
   *
   * @tparam T The type of the data to broadcast.
   ...
   *
   * This function sends execution information and arguments to the host for
   * processing, triggered by a specific signal.
   *
   * @tparam Args Variadic template types for the arguments.
   * @param info An `ExecInfo` object containing execution metadata.
   * @param sign A `SIGNALS` enumeration indicating the signal to trigger the
   * task.
   ...
   *
   * This function collects raw data of type `T` from all processes into a
   * single vector on the root process. It performs no safety checks and assumes
   * the input pointers and sizes are valid.
   *
   ...
   * @param root The identifier of the root process that gathers the data
   * (default is 0).
   ...
   *
   * This function collects data stored in a `std::span` from all processes into
   * a single vector on the root process.
   *
   * @tparam T The type of the data to gather.
   ...
   *
   * This function collects data of type `T` from all processes, performs a
   * reduction operation (sum), and returns the result to the root process.
   *
   * @tparam T A type satisfying the `NumberType` concept.
   * @param data The local data to be reduced and gathered.
   * @param root The identifier of the root process that gathers and reduces the
   * data (default is 0).
   ...
   *
   * This function collects vectors of data from all processes into a single
   * vector on the root process.
   *
   * @tparam T The type of the data to gather.
