...
def mk_flowmap(dest, name):
    ...
    _case = cmtool.generate.generate_0D_set_from_fraction(
        outfolder, 20e-3, 5 / 100, eps_turb
    )
    cmtool.cma_case.mk_write_cma_case(
        f"{outfolder}/cma_case", [1, 0, 0], description=f"{name}", cma_path=_case
    )
    return outfolder
...
def run(params, out, name, cma_path):
    ...
    handle_module.set_initial_concentrations(
        handle, liquid_concentration_0, gas_concentration_0
    )
