...
def copy_and_compress(h5_source, h5_dest, compression="gzip", compression_opts=9):
    ...
    for k in h5_source.attrs:
        h5_dest.attrs[k] = h5_source.attrs[k]
    ...
    for key in h5_source:
        ...
        if isinstance(_item, h5py.Dataset):
            ...
            copy_and_compress(
                _item, group, compression=compression, compression_opts=compression_opts
            )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Compress HDF5 files in a directory using GZIP compression."
    )
    parser.add_argument(
        "root", type=str, help="Root directory containing HDF5 files to compress."
    )
